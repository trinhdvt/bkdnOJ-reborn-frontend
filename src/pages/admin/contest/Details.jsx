...
import { FaGlobe, FaRegTrashAlt, FaChartLine, FaRegCalendarAlt, FaInfo } from 'react-icons/fa';
...
class AdminContestDetails extends React.Component {
  ...
  render() {
    ...
    if (loaded && !errors) {
      if (data.standing_outdated_reason)
        standingRecomputeCss = 'danger'
        ...
        <div className="contest-details">
          ...
          <div className="m-1 p-1">
            ...
            <div className="flex-center">
              <div className="p-1 border flex-center-col">
                <span className="d-flex">
                  ...
                  <OverlayTrigger placement="bottom" overlay={ 
                      <Tooltip>
                        <ul style={{fontSize: "14px"}} className="text-left m-0 pl-3 w-100">
                          <li>Contest là contest <code>{data.is_rated ? "Xếp hạng" : "Không xếp hạng"}</code>.</li>
                          <li>Rating Floor là <code>{data.rating_floor ? data.rating_floor : "0"}</code>.</li>
                          <li>Rating Ceiling là <code>{data.rating_ceiling ? data.rating_ceiling : "+∞"}</code>.</li>
                        </ul>
                      </Tooltip> 
                    }>
                    <Button size="sm" type="button" variant="light" className="ml-1">
                      <FaInfo/>
                    </Button>
                  </OverlayTrigger>
                </span>
              ...
              <div className="p-1 border flex-center-col">
                <span className="d-flex">
                  <Button className="btn-svg" size="sm" variant={this.state.recomputeDisabled ? 'light' : standingRecomputeCss} 
                    disabled={this.state.recomputeDisabled}
                    onClick={()=>{
                      contestAPI.recomputeContestStanding({ key: data.key })
                      .then((res) => {
                        toast.success("OK đã queue tác vụ.")
                        this.setState({ recomputeDisabled: true })
                      })
                      .catch((err) => {
                        toast.error(`Không thể tính lại bảng điểm. (${err.response.status})`)
                      })
                    }}>
                      <FaRegCalendarAlt size={16}/><span className="d-none d-md-inline">Tính lại Standing</span>
                  </Button>
                  <OverlayTrigger placement="bottom" overlay={ 
                      <Tooltip>
                        <ul style={{fontSize: "14px"}} className="text-left m-0 pl-3 w-100">
                          <li>Contest: <code>{data.modified}</code>.</li>
                          <li>Standing: <code>{data.standing_date}</code>.</li>
                          { data.standing_outdated_reason ?
                            <li><span>Standing <strong>cần được tính lại</strong>.
                            Lý do: <code>{data.standing_outdated_reason}</code>.</span></li>
                            : <li><span>
                              Hệ thống không phát hiện ra lý do cần tính lại. {
                                data.modified !== data.standing_date ? "Tuy nhiên, contest vừa được update gì đó có thể không liên quan đến standing." : ""
                              }
                            </span></li>
                          }
                        </ul>
              ...
              <div className="p-1 border flex-center-col">
                <span className="d-flex">
                  <Button className="btn-svg" size="sm" variant="light" disabled
                    onClick={()=>{}}>
                      <BsUpcScan size={16}/><span className="d-none d-md-inline">Kiểm tra Đạo code</span>
                  </Button>
                </span>
              </div>
            </div>
